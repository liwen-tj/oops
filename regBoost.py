...
class RegBoost:
    ...
    def get_model(self, data, Y, layer):
        ...
        for i in range(self.learner_num):
            ...
            if(layer-1 == 0 or data_p.__len__() < self.min_sample): # 已经达到最大层数
                model_p = None
            else:
                model_p = self.get_model(data_p, Yp, layer-1)

            if(layer-1 == 0 or data_n.__len__() < self.min_sample): # 已经达到最大层数
                model_n = None
            else:
                model_n = self.get_model(data_n, Yn, layer-1)
            
            # data_p, data_n, Yp, Yn都是list数据类型
            model.append((learner, fea, data_p, data_n, model_p, model_n))
        return model
